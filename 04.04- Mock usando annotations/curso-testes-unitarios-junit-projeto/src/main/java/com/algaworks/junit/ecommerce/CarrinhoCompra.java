...
public class CarrinhoCompra {
	...
	public CarrinhoCompra(Cliente cliente, List<ItemCarrinhoCompra> itens) {
		Objects.requireNonNull(cliente); // Solta um NullPointeException
		Objects.requireNonNull(itens); // Solta um NullPointeException
	...
	public List<ItemCarrinhoCompra> getItens() {
		...
		List<ItemCarrinhoCompra> novaListaDeItensDaCompra = new ArrayList<>(itens);
		return novaListaDeItensDaCompra;
	}
	...
	public void adicionarProduto(Produto produto, Integer quantidade) {
		...
		Objects.requireNonNull(produto); // Solta um NullPointeException
		if (quantidade == null || quantidade <= 0) {
			throw new IllegalArgumentException("Quantidade não pode ser nuloe nao pode ser menor ou igual" +
					"a zero");
		}
